...
class BatchResults:
    def __init__(self):
      ...
      self.target_board_2_words = [line[:2] for line in self.target_board_8_words]
      self.target_board_3_words = [line[:3] for line in self.target_board_8_words]
    ...
    def fixed_batch_results(self, target_board, board_name):
      ...
      while i < len(target_board):
        try:
          ...
          model_result_glove_embedding_nn  = self.model_glove.select_best_hint_from_embeddings_and_neighbors(target_words, default_avoid)
          print(f"Suggested Glove NN hint: {model_result_glove_embedding_nn["best_hint"]}\n")
          
          model_result_glove_embedding = self.model_glove.select_best_hint_from_embeddings(target_words, default_avoid)
          ...
          self.results.append(
              {
                  ...
                  "Expected embedding word BERT": model_result_bert_embedding_nn["best_hint"],
                  "Similarity embedding BERT":model_result_bert_embedding_nn["best_score"],
    ...
    def get_fixed_batch_results(self):
      ...
      if self.results:
          ...
          file_name = f"results/{uuid.uuid4()}_results_with_sections.csv"
          df.to_csv(file_name, index=False)
          print(f"All results saved in {file_name}")
      else:
          print("Empty results.")
